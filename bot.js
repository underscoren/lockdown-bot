...
bot.on("interactionCreate", async interaction => {
    ...
    try {
        ...
        if (interaction.deferred) 
            interaction.editReply({content: "Error while processing command", ephemeral: true});
        else
            interaction.reply({content: "Error while processing command", ephemeral: true});
    }
});
...
bot.updateCommands = function() {
    for (const guild of bot.guilds.cache.values()) {
        debug(`Setting commands for ${guild.name}`);
